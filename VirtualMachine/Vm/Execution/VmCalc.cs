...
public static class VmCalc
{
    ...
    {
        if ((a.Type & Number) != 0)
            return VmValue.Create(a.Get<double>().EqualWithAccuracy(b.Get<double>(), accuracy) ? 1.0 : 0.0, Number);
        return VmValue.Create(a.GetRef<string>() == b.GetRef<string>() ? 1.0 : 0.0, Number);
    }
